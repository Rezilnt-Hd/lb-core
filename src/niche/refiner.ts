...
const REFINER_MODEL_ID =
  process.env.BEDROCK_MODEL_REFINER || 'us.anthropic.claude-haiku-4-5-20251001-v1:0';
 ...
 */
export async function resolveRefinedNiche(
  coarseNiche: string,
  ...
  for (const { alias, niche } of candidates) {
    if (bt.includes(alias.toLowerCase())) return niche;
  }
