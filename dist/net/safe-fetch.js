 ...
 */
export async function assertPublicUrl(rawUrl, lookup = defaultLookup) {
    let u;
    try {
        u = new URL(rawUrl);
    }
    ...
    if (ips.length === 0)
        throw new SsrfError(`no DNS resolution: ${host}`);
    for (const ip of ips) {
        if (isBlockedIp(ip))
            throw new SsrfError(`${host} resolves to blocked address ${ip}`);
    }
    return u;
}
function enforceSize(res, maxBytes) {
